/* =====================================================
   perigal-engine.js  v9  (Perigal PUSAT, mesin DFS terverifikasi)
   cfg diterima tetapi diabaikan (selalu pusat).
   ===================================================== */
function perigalGeometry(a, b, cfg, viewW, viewH, pad) {
    ...
    signs.forEach(([sH, sN]) => {
        ...
        if (poly.length >= 3 && Math.abs(area(poly)) > 1e-9) pieces.push({ poly, t: null });
    });

    const tol = 0.02 * c, samples = [], stepS = c / 22;
    for (let x = minX; x <= maxX; x += stepS)
        for (let y = minY; y <= maxY; y += stepS) {
            const p = { x, y };
    ...
    if (!solution) {
        valid = false;
        console.warn('⚠️ Perigal: tidak ada solusi untuk triple ' + a + ',' + b);
        pieces.forEach(pc => pc.t = { x: 0, y: 0 });
    } else {
        pieces.forEach((pc, i) => pc.t = solution[i]);
        console.log('✅ TILING OK (Perigal pusat ' + a + ',' + b + ')');
    }
